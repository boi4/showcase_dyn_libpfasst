...
/*
 *  C++ functions called from Fortran to manipulate Hypre vector and solver
 * objects
 */
...
int HypreSolverGetNumRowsLevel(HypreSolver *hypre_solver,
  ...
  if (glob_spacial_coarsen_flag == 1) {
    ...
    level_index = PfasstToHypreLevelIndex(pfasst_level_index,
                                          hypre_solver->GetNumLevels());
    num_rows = hypre_solver->nrows;
  }
  return num_rows;
}

int HypreSolverGetExtentLevel(HypreSolver *hypre_solver, int pfasst_level_index,
                              int i) {
  int level_index, extent;
  level_index =
      PfasstToHypreLevelIndex(pfasst_level_index, hypre_solver->GetNumLevels());
  if (glob_spacial_coarsen_flag == 1) {
    level_index = 0;
    switch (i) {
    case 0:
...
void HypreVectorCreate(HypreVector **hypre_vector, int num_grid_points,
                       MPI_Fint fspace_comm, int space_dim, int nrows, int ilower0,
                       int ilower1, int iupper0, int iupper1) {
  if (*hypre_vector == nullptr) {
    ...
    *hypre_vector = new HypreVector(num_grid_points, 0.0, newcomm, space_dim,
                                    nrows, extents);
  }
}
...
void HypreVectorPrint(HypreVector *hypre_vector) { hypre_vector->Print(); }

void HypreVectorDump(HypreVector *hypre_vector, char *fname) {
  hypre_vector->Dump(fname);
}

void HypreSolverInit(HypreSolver **hypre_solver, int pfasst_level_index,
                     int num_grid_points, MPI_Fint fspace_comm, int space_dim,
                     int max_iter, int max_levels, int spacial_coarsen_flag) {
  ...
  if (spacial_coarsen_flag == 1) {
    if (level_index == 0) {
      ...
      //*hypre_solver = new HypreSolver(newcomm, space_dim, max_iter,
      //max_levels, num_grid_points);
      //(*hypre_solver)->A_imp = (*hypre_solver)->SetupMatrix(num_grid_points,
      //level_index, spacial_coarsen_flag, 0);
      //(*hypre_solver)->A_exp = (*hypre_solver)->SetupMatrix(num_grid_points,
      //level_index, spacial_coarsen_flag, 1);
    ...
    if (*hypre_solver == nullptr) {
      ...
      *hypre_solver = new HypreSolver(newcomm, space_dim, max_iter, max_levels,
                                      num_grid_points);
      (*hypre_solver)
          ->SetupMatrix(&((*hypre_solver)->A_exp), num_grid_points, level_index,
                        spacial_coarsen_flag, 0, 0.0);
      (*hypre_solver)
          ->SetupMatrix(&((*hypre_solver)->A_imp), num_grid_points, level_index,
                        spacial_coarsen_flag, 0, 0.0);
      (*hypre_solver)->x = (*hypre_solver)->SetupVector();
      (*hypre_solver)->b = (*hypre_solver)->SetupVector();
    }
...
void HypreImplicitSolverInit(HypreSolver **hypre_solver, int pfasst_level_index,
                             int num_grid_points, MPI_Fint fspace_comm, int space_dim,
                             int max_iter, int max_levels, double dtq) {
  int level_index = PfasstToHypreLevelIndex(pfasst_level_index, max_levels);
  if (glob_spacial_coarsen_flag == 1) {
    if (level_index == 0) {
    }
    ...
    (*hypre_solver)
        ->SetupMatrix(&((*hypre_solver)->A_imp), num_grid_points, level_index,
                      glob_spacial_coarsen_flag, 1, dtq);
    (*hypre_solver)
        ->SetupStructSolver(&((*hypre_solver)->A_imp),
                            &((*hypre_solver)->solver_imp),
                            &((*hypre_solver)->precond_imp));
  }
}
...
void HypreSolverFEval(HypreSolver *hypre_solver, HypreVector *y, double t,
                      int pfasst_level_index, HypreVector *f, int piece) {
  int level_index =
      PfasstToHypreLevelIndex(pfasst_level_index, hypre_solver->GetNumLevels());
...
void HypreSolverFComp(HypreSolver *hypre_solver, HypreVector *y, double t,
                      double dtq, HypreVector *rhs, int pfasst_level_index,
                      HypreVector *f, int piece) {
  if (piece != 2) {
  ...
  double *f_values = (double *)malloc(nrows * sizeof(double));
  hypre_solver->FComp(&y_values, t, dtq, rhs->GetBoxValues(), level_index,
                      &f_values);
...
double HypreMaxErr(HypreVector *hypre_vector, double t, double init_cond) {
  ...
  // MPI_Allreduce(&error_inner_prod, &error_L2norm, 1, MPI_DOUBLE, MPI_SUM,
  // hypre_vector->comm); error = sqrt(error_L2norm);

  return error;
}

void HypreRestrict(HypreVector *y_f, HypreVector *y_c, int pfasst_f_level,
                   int pfasst_c_level) {
  if (glob_spacial_coarsen_flag == 1) {
    int f_level = PfasstToHypreLevelIndex(pfasst_f_level,
                                          glob_hypre_solver->GetNumLevels());
    int c_level = PfasstToHypreLevelIndex(pfasst_c_level,
                                          glob_hypre_solver->GetNumLevels());
    glob_hypre_solver->Restrict(y_f->GetVector(), y_c->GetVector(), f_level,
                                c_level);
  } else {
...
void HypreProlong(HypreVector *y_f, HypreVector *y_c, int pfasst_f_level,
                  int pfasst_c_level) {
  if (glob_spacial_coarsen_flag == 1) {
    int f_level = PfasstToHypreLevelIndex(pfasst_f_level,
                                          glob_hypre_solver->GetNumLevels());
    int c_level = PfasstToHypreLevelIndex(pfasst_c_level,
                                          glob_hypre_solver->GetNumLevels());
    glob_hypre_solver->Prolong(y_f->GetVector(), y_c->GetVector(), f_level,
                               c_level);
  } else {
...
void HypreSolverSetLevelData(HypreSolver **y, HypreSolver *x,
                             int pfasst_level_index) {
  int level_index;
  if (glob_spacial_coarsen_flag == 1) {
    // level_index = PfasstToHypreLevelIndex(pfasst_level_index,
